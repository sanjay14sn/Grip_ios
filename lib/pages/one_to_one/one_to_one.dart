...
Future<File?> _compressIfNeeded(File file) async {
  // Skip compression if file is already < 1MB
  ...
  // Ensure result is treated as File
...
void _handleSubmitOneToOne() async {
  ...
  if (toMemberId == null || _pickedImage == null || selectedLocation == null) {
  ...
  // compress again before uploading (just to be safe)
  final compressedFile = await _compressIfNeeded(_pickedImage!);
  ...
  if (response.isSuccess) {
    ...
    ToastUtil.showToast(context, '❌ Failed to submit slip.');
  }
