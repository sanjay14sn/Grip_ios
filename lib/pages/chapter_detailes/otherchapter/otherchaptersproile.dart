...
class _OtherChapterPageState extends State<OtherChapterPage> {
  ...
  setState(() => isLoading = true);
  print('🔄 Fetching CID members for chapterId: ${widget.chapterId}');
  ...
  while (attempt < maxRetries && !success) {
    ...
    final response =
        await PublicRoutesApiService.fetchMembersByChapter(widget.chapterId);
    print('➡️ fetchMembersByChapter Response: ${response.data}');
    print('➡️ Status: ${response.isSuccess}');
    ...
    if (response.isSuccess && response.data != null) {
      try {
        ...
        final List<othersMemberModel> members =
            rawList.map((e) => othersMemberModel.fromJson(e)).toList();
        print('✅ Members fetched successfully: ${members.length} members');

        // ✅ Also fetch CID members
        final cidResponse =
            await PublicRoutesApiService.fetchCidDetailsList(widget.chapterId);
        print('➡️ fetchCidDetailsList Response: ${cidResponse.data}');
        print('➡️ CID Status: ${cidResponse.isSuccess}');
        ...
        if (cidResponse.isSuccess && cidResponse.data != null) {
          ...
          print('✅ CID Members fetched successfully: ${cidMembers.length} members');
        } else {
          print('⚠️ Failed to fetch CID members');
        }
        ...
        print('🎉 All members and CID members loaded successfully');
        success = true;
      } catch (e, stack) {
        print('❌ Error while parsing members: $e');
        print(stack);
        setState(() => isLoading = false);
        break;
      }
